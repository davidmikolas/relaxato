...
from grand_minimizer_v20 import Island, E_hexi, Island_from_HOC
import json, yaml #, pickle
from datetime import datetime

### add initial and final cm positions and velocities to summary_dict   DONE!
### add filename based on start time (for all output filenames)   DONE!
### add a "finished" criteria flag for summary   DONE!
### convert summary_dict values to normal floats and lists for YAML output   DONE!
...
rotations = [1, 2, 29, 30] # list(range(0, 9)) + list(range(30-8, 31))
lattice_constants = [1.35] # np.linspace(1.32, 1.38, 3)

summary_dict = dict()
start_dict = dict()

# https://docs.python.org/3/library/datetime.html truncate to milliseconds
begin_time_string = datetime.today().strftime('%Y-%m-%d at %H.%M.%S.%f')[:-3]
...
with open('start_dict ' + begin_time_string + '.json', 'w') as outfile:
    json.dump(start_dict, outfile)
with open('summary_dict ' + begin_time_string + '.json', 'w') as outfile:
...
with open('start_dict ' + begin_time_string + '.yaml', 'w') as outfile:
    yaml.dump(start_dict, outfile)
with open('summary_dict ' + begin_time_string + '.yaml', 'w') as outfile:
