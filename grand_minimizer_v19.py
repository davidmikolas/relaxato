...
class Island(): # asdf In the future, branch when kind== 'hoc'
    def __init__(self, a=1, R=0, strain=0, randomize=None, sig_xy=0.,
        ...
        self.E_surf = E_surf
        if nmin != None and not is_hoc:   ##### ##### HsuKai fixed
    ...
    def start_report(self):
        ...
        # these don't have to be calculated, so just save them
        things = ('a', 'R', 'strain', 'randomize', 'sig_xy',
                  'iseed', 'kind', 'nmax', 'nmin',
                  'E_surf_kwargs', 'E_bond_alpha', 'grad_surf_d',
                  'grad_surf')
        ...
        self.start_dict['xy_offset'] = tuple(self.xy_offset)

        self.report_dict = {'start': self.start_dict} # initialize report_dict
            

    def initial_report(self):
        # these have to be calculated, so do one at a time
        ...
        self.summary_dict['bond_lengths_mean_initial'] = self.bond_lengths_initial_mean
        ...
        self.summary_dict['angles_initial_mean'] = self.angles_initial_mean
        self.summary_dict['angles_initial_weighted_mean'] = self.angles_initial_weighted_mean
        ...
        self.summary_dict['energy_bonds_initial'] = self.energy_bonds_initial_total
        self.summary_dict['energy_surface_initial'] = self.energy_surface_initial_total
        self.summary_dict['energy_total_initial'] = self.energy_initial_total
    ...
    def final_report(self):
        ...
        self.summary_dict['bond_lengths_mean_final'] = self.bond_lengths_final_mean
        ...
        self.summary_dict['angles_final_mean'] = self.angles_final_mean
        self.summary_dict['angles_final_weighted_mean'] = self.angles_final_weighted_mean
        ...
        self.summary_dict['energy_bonds_final'] = self.energy_bonds_final_total
        self.summary_dict['energy_surface_final'] = self.energy_surface_final_total
        self.summary_dict['energy_total_final'] = self.energy_final_total
...
# run once for sure
        ...
        y = answer['y']
        # print('once - y.shape: ', y.shape)
        state_vectors = [y]
        ending_state_vector = y[:, -1]
        # print('once - ending_state_vector.shape: ', ending_state_vector.shape)
        ...
        while self.n_check <= n_check_max and not self.stopped:
            ...
            # print('self.n_check: ', self.n_check)
            ...
            y = answer['y']
            ending_state_vector = y[:, -1]
            # print(self.n_check, 'ending_state_vector.shape: ', ending_state_vector.shape)
        ...
        n_points = state_vectors.shape[1]
        # remove self. from state_vectors and view
        view = state_vectors.reshape(4, -1, n_points)
