...
class Island(): # asdf In the future, branch when kind== 'hoc'
    def __init__(self, a=1, R=0, strain=0, randomize=None, sig_xy=0.,
                 xy_offset=(0., 0.), iseed=None, kind='hex', nmax=3, nmin=None,
                 E_surf=None, E_surf_kwargs=None, grad_surf='central',
                 E_bond_alpha=None, grad_surf_d=0.00001):
        ...
        self.E_surf = E_surf
        if nmin != None and not is_hoc:   ##### H-K fixed
    ...
    def initialize_vecs(self):
        ...
        self.vecs_rot =  (self.vecs[:, None] * rotm).sum(axis=2) # I'm almost sure!
    ...
    def prune_ij(self):
        ...
        self.origin_atom = np.where(np.all(self.ij.T == np.zeros(2),axis=1))
    ...
    def get_grad_surf(self, xy, d): # central difference
    ...
    def get_angles(self, xy):
        use = np.ones(xy.shape[1], dtype=bool)
        use[self.origin_atom] = False
        x0, y0 = self.xy_original[:, use]
        x, y = xy[:, use]
        ...
        angles = np.mod(dangles + 180, 360) - 180. + self.R
    ...
    def start_report(self):
        ...
        things = ('a', 'R', 'strain', 'randomize', 'sig_xy',
                  'iseed', 'kind', 'nmax', 'nmin',
                  'E_bond_alpha', 'grad_surf_d',
                  'grad_surf')

        for thing in things:
            self.start_dict[thing] = getattr(self, thing)
        
        self.start_dict['E_surf_name'] = self.E_surf.__name__
        self.start_dict['xy_offset'] = [float(q) for q in self.xy_offset]
        self.start_dict['E_surf_kwargs'] = [list(q) for q in
                                            self.E_surf_kwargs.items()]

        # initialize report_dict
        self.report_dict = {'start': self.start_dict}
    ...
    def initial_report(self):
        ...
        self.report_dict['initial'] = self.initial_dict
        self.report_dict['summary'] = self.summary_dict

        # bond lengths
        self.bond_lengths_initial = self.get_bond_distances(self.xy)
        self.bond_lengths_initial_mean = self.bond_lengths_initial.mean()
        self.initial_dict['bond_lengths'] = self.bond_lengths_initial
        self.summary_dict['bond_lengths_mean_initial'] = float(
            self.bond_lengths_initial_mean)
        
        # angles
        angles, mean_angles, weighted_mean_angles = self.get_angles(self.xy)
        self.angles_initial = angles
        self.angles_initial_mean = mean_angles
        self.angles_initial_weighted_mean = weighted_mean_angles
        self.initial_dict['angles'] = self.angles_initial
        self.summary_dict['angles_initial_mean'] = float(self.angles_initial_mean)
        self.summary_dict['angles_initial_weighted_mean'] = float(
            self.angles_initial_weighted_mean)

        # energies
        self.energies_bond_initial = self.get_bond_energies(self.xy)
        self.energies_surface_initial = self.get_surface_energies(self.xy)
        self.initial_dict['energies_surface'] = self.energies_bond_initial
        self.initial_dict['energies_bond'] = self.energies_surface_initial
        self.energy_bonds_initial_total = self.energies_bond_initial.sum()
        self.energy_surface_initial_total = self.energies_surface_initial.sum()
        self.energy_initial_total = (self.energy_bonds_initial_total +
                                     self.energy_surface_initial_total)

        self.summary_dict['energy_bonds_initial'] = float(
            self.energy_bonds_initial_total)
        self.summary_dict['energy_surface_initial'] = float(
            self.energy_surface_initial_total)
        self.summary_dict['energy_total_initial'] = float(
            self.energy_initial_total)
    ...
    def final_report(self):
        ...
        self.report_dict['final'] = self.final_dict
        self.report_dict['trajectories'] = self.trajectory_dict
        
        # bond lengths
        self.bond_lengths_final = self.get_bond_distances(self.xy_final)
        self.bond_lengths_final_mean = self.bond_lengths_final.mean()
        self.final_dict['bond_lengths'] = self.bond_lengths_final
        self.summary_dict['bond_lengths_mean_final'] = float(
            self.bond_lengths_final_mean)

        # angles
        angles, mean_angles, weighted_mean_angles = self.get_angles(self.xy_final)
        self.angles_final = angles
        self.angles_final_mean = mean_angles
        self.angles_final_weighted_mean = weighted_mean_angles
        self.final_dict['angles'] = self.angles_final
        self.summary_dict['angles_final_mean'] = float(self.angles_final_mean)
        self.summary_dict['angles_final_weighted_mean'] = float(
            self.angles_final_weighted_mean)

        # energies
        self.energies_bond_final = self.get_bond_energies(self.xy_final)
        self.energies_surface_final = self.get_surface_energies(self.xy_final)
        self.final_dict['energies_surface'] = self.energies_bond_final
        self.final_dict['energies_bond'] = self.energies_surface_final
        self.energy_bonds_final_total = self.energies_bond_final.sum()
        self.energy_surface_final_total = self.energies_surface_final.sum()
        self.energy_final_total = (self.energy_bonds_final_total +
                                     self.energy_surface_final_total)
        self.summary_dict['energy_bonds_final'] = float(
            self.energy_bonds_final_total)
        self.summary_dict['energy_surface_final'] = float(
            self.energy_surface_final_total)
        self.summary_dict['energy_total_final'] = float(
            self.energy_final_total)

        # trajectories and initial + final values
        self.trajectory_dict['positions'] = self.trajectories
        self.trajectory_dict['velocities'] = self.velocities
        self.max_velocity = np.sqrt((self.velocities**2).sum(axis=1)).max(axis=0)
        self.trajectory_dict['max_velocity'] = self.max_velocity
        self.trajectory_dict['position_cm'] = self.trajectory_cm
        self.trajectory_dict['velocity_cm'] = self.velocity_cm

        self.summary_dict['position_cm_initial'] = self.trajectory_cm[:, 0].tolist()
        self.summary_dict['position_cm_final'] = self.trajectory_cm[:, -1].tolist()
        self.summary_dict['velocity_cm_initial'] = self.velocity_cm[:, 0].tolist()
        self.summary_dict['velocity_cm_final'] = self.velocity_cm[:, -1].tolist()

        # other
        self.summary_dict['process_time'] = self.process_time
        self.summary_dict['n_check'] = self.n_check
        self.summary_dict['n_check_max'] = self.n_check_max
        self.summary_dict['stopped'] = self.stopped
        self.summary_dict['n_timepoints'] = self.n_points
        

    def solve_as_ivp_smart(self, dt_check, n_check_max, n_eval_per,
                           v_stop=-1, rtol=1E-06, method='DOP853',
                           damping=1.0, dense_output=False):
        ...
        self.n_check_max = int(n_check_max)
        ...
        tvals = np.linspace(0, dt_check, n_eval_per+1)

        self.stopped = False   # initialize
...
# run once for sure
        ...
        y = answer['y']
        state_vectors = [y]
        ending_state_vector = y[:, -1]

        t_evals = [t_eval]
        self.ivp_messages = [answer['message']]
        self.ivp_success = [answer['success']]
        self.nfevs = [answer['nfev']]

        self.n_check = 1

        while self.n_check <= n_check_max and not self.stopped:

            t_eval = self.n_check * dt_check + tvals
            t_evals.append(t_eval[1:])
            t_span = t_eval.min(), t_eval.max()
            answer = solve_ivp(deriv, t_span=t_span, y0=ending_state_vector,
                            method=method, args=args, t_eval=t_eval[1:],
                            rtol=rtol, dense_output=dense_output,
                               events=None)

            y = answer['y']
            ending_state_vector = y[:, -1]
            state_vectors.append(y)
            self.ivp_messages.append(answer['message'])
            self.ivp_success.append(answer['success'])
            self.nfevs.append(answer['nfev'])
            n_points = y.shape[1]
            velocities = answer['y'].reshape(2, -1, n_points)[1]
            if np.abs(velocities).max() < v_stop:
                self.stopped = True
            self.n_check += 1

        self.process_time = time.process_time() - t_start

        self.t_eval = np.hstack(t_evals)

        state_vectors = np.hstack(state_vectors)
        self.n_points = state_vectors.shape[1]

        view = state_vectors.reshape(4, -1, self.n_points)
        ...
        self.xy_final = self.trajectories[..., -1].T
        self.vxy_final = self.velocities[..., -1].T
    ...
    def solve_as_ivp(self, t_eval, rtol=1E-03, method='DOP853', damping=1.0, 
        ...
        self.n_points = self.state_vectors.shape[1] # this one
...
# stand-alone functions that can be imported
